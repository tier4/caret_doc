...
| ------ | -------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 1      | Before the application starts                      | Execute the record command once for any domain (or without specifying a domain) | All domains are subject to measurement.                             |- The data can be analyzed as long as there are no duplicate namespace/node_name pairs.</br>- Only domains with LD_PRELOAD set can be analyzed. |
| 2      | Before the application starts                      | Execute the record command for each domain                                      | Not supported                                                       |                                                                                                               |
| 3      | While the application is running                   | Execute the record command once only for the target domain.                     | The domain where the record was executed is the measurement target. | The presence or absence of the LD_PRELOAD setting is generally irrelevant outside the target domain.          |
| 4      | While the application is running                   | Execute the record command for each domain                                      | Not supported                                                       |                                                                                                               |
