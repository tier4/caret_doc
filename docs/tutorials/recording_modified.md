...
# In case CARET/rclcpp is applied to all packages, there is no warning
...
CARET depends on LTTng for tracing applications. LTTng session has to be started before a target application runs.
...
Note that if you execute the target application before starting LTTng session will result in a lack of trace points.
...
See [Validation](../validatin) to find more details.
